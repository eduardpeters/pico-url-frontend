...
function UrlEditForm({ shortUrl, originalUrl, userToken, closeForm }: UrlEditFormProps) {
    ...
    return (
        <form className="url__form" onSubmit={(event) => handleUrlEdit(event)}>
            <label className="url__label" htmlFor="url-input">
                New URL:
            </label>
            <input
                ...
                type="url"
                value={newUrl}
                placeholder="new looong url"
                onChange={(event) => setNewUrl(event.target.value)}
            ></input>
            <button className="url__button url__button-replace" type="submit">
                Replace!
            </button>
            <button
                className="url__button url__button-cancel"
                type="button"
                onClick={() => closeForm()}
            >
                Nevermind
